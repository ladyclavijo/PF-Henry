...
export default function MyAccount() {
  ...
  const filteredArray = boughtList.filter((book) =>
    historyItems?.includes(book.id)
  );

  const finalArray = filteredArray.map((book1) => {
    const matchingBook = historyNew.find((book2) =>
      book2.items.some((item) => item.id === book1.id)
    );
  ...
  return (
    ...
    >
      ...
      <div className="mt-5 ml-5 flex" style={styles.container}>
        ...
        {editing ? (
          <div
            className="w-96 bg-[#9dc8c5] p-4 items-center shadow rounded-md"
            style={styles.container}
          >
          ...
          <div className="user-info" style={styles.container}>
            ...
            >
              ...
              <div className="font-bold text-2xl">{finalUser?.name}</div>
        ...
        {show ? (
          <div
            className="bg-[#9dc8c5] ml-8 mt-4 items-start flex flex-row m-4 p-3 shadow rounded-md"
            style={styles.container}
          >
            <button
              className="font-bold"
                  ...
                  <Link to={`/book/ ${el.id}`}>
                    <div className="mb-4" key={el.id}>
                      ...
                      <p className="text-sm italic">
                        Bought date: <br /> {el.date.slice(0, 10)},{" "}
                        {el.date.slice(11, 16)}
                      </p>
                    </div>
                  </Link>
          ...
          <div
            className=" ml-8 mt-4 items-start flex flex-row m-4"
            style={styles.container}
          >
            <button
              className="font-bold bg-[#9dc8c5] p-3 shadow rounded-md"
